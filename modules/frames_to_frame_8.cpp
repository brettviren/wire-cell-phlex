...
// modules/frames_to_frame_8.cpp
//
// PHLEX algorithm module: 8 x IFrame -> IFrame fan-in node
// (FaninExecutor<IFrame,IFrame,8>).  Node name "wcph_frames_to_frame_8".
//
...
// modules/register_shapes.hpp).  Consumes 8 Frames (one per "inputs"
// selector), merges them (FrameFanin) and produces one merged Frame.
//
// Config keys: wct_config (required), inputs (8 selectors), outputs (1),
...
{
    wcphlex::register_fanin<WireCell::IFrame, WireCell::IFrame, 8>(m, config);
}

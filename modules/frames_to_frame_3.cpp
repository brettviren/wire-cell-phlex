...
// modules/frames_to_frame_3.cpp
//
// PHLEX algorithm module: 3 x IFrame -> IFrame fan-in node
// (FaninExecutor<IFrame,IFrame,3>).  Node name "wcph_frames_to_frame_3".
//
...
// modules/register_shapes.hpp).  Consumes 3 Frames (one per "inputs"
// selector), merges them (FrameFanin) and produces one merged Frame.
//
// Config keys: wct_config (required), inputs (3 selectors), outputs (1),
...
{
    wcphlex::register_fanin<WireCell::IFrame, WireCell::IFrame, 3>(m, config);
}

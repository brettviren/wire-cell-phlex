...
// modules/frames_to_frame_4.cpp
//
// PHLEX algorithm module: 4 x IFrame -> IFrame fan-in node
// (FaninExecutor<IFrame,IFrame,4>).  Node name "wcph_frames_to_frame_4".
//
...
// modules/register_shapes.hpp).  Consumes 4 Frames (one per "inputs"
// selector), merges them (FrameFanin) and produces one merged Frame.
//
// Config keys: wct_config (required), inputs (4 selectors), outputs (1),
